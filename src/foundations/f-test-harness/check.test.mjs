...
fs.rmSync(path.join(root, "src", "interactions", "i-orphan"), { recursive: true, force: true });
...
// Plan-only PR: committed create_node .gitkeep for a ready Interaction must pass.
// Only i-feature (ready) — f-core is already in-progress and would mask the exemption.
git("add", "src/interactions/i-feature/.gitkeep");
git("commit", "-m", "i-feature scaffold");
...
// Ship foundation + interaction, then committed dirty vs base must pass
...
fs.writeFileSync(
  path.join(root, "src", "interactions", "i-dead", "wip.ts"),
  "export const nope = 1;\n"
);
r = runCheck(["--base", cancelBase]);
if (r.status === 0 || !(r.stderr || r.stdout).includes("Uncommitted")) {
...
fs.rmSync(path.join(root, "src", "interactions", "i-dead", "wip.ts"), { force: true });
...
await call("update_node_status", { node_id: "i-evolve", new_status: "in-review" });
await call("update_node_status", { node_id: "i-evolve", new_status: "ship" });

const evolveBase = git("rev-parse", "HEAD").stdout.trim();
await call("open_next", { node_id: "i-evolve" });

// next draft + package-root .gitkeep alone is allowed (same plan-scaffold exemption)
fs.writeFileSync(path.join(root, "src", "interactions", "i-evolve", ".gitkeep"), "\n");
git("add", "src/interactions/i-evolve/.gitkeep");
...
fs.writeFileSync(
  path.join(root, "src", "interactions", "i-evolve", "v2.ts"),
  "export const v2 = true;\n"
);
git("add", "src/interactions/i-evolve/v2.ts");
...
if (freeCreate.implementation || fs.existsSync(path.join(root, "src", "interactions", "i-free-extra"))) {
  failures++;
...
} else if (freeCreate.changed_files?.some((f) => String(f).includes("src/interactions/"))) {
  failures++;
...
const freeBody = fs.readFileSync(
  path.join(root, "mindplan", "interactions", "i-free-extra", "current.mdx"),
  "utf-8"
);
if (!freeBody.includes("layout-free") || freeBody.includes("Implement code only under `src/interactions/<id>/`")) {
  failures++;
  console.log("FAIL layout-free scaffold body should not prescribe src/interactions path");
} else console.log("ok   layout-free scaffold body mentions layout-free");

const freeImpl = await call("get_node_implementation", { node_id: "i-feature" });
...
try {
  await call("get_node_implementation", { node_id: "i-feature" });
  failures++;
